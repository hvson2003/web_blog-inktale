...
// Clears the image preview by removing the 'show' class from th preview container
...
const $progessBar = document.querySelector('[data-progress-bar]');
...
const updateBasicInfo = async (event) => {
    ...
    // Show progress bar
    $progessBar.classList.add('loading');
    ...
    if (response.ok) {
        ...
        $basicInfoSubmit.removeAttribute('disabled');
        $progessBar.classList.add('loading-end');
        Snackbar({ message: 'Your profile has been updated !' });
        window.location.reload();
    }

    // Handle case where response status is 400 (Bad resquest)
...
const deleteAccount = async (req, res) => {
    ...
    // Show account delete request in server
