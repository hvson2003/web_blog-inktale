 ...
 * @throws {Error} - Throws an error if there's an issue during process.
 */
    ...
    try {
        ...
        if (username) {
            if (await User.exists({ username })) {
                return res.status(400).json({ message: 'Sorry, this username is already taken. Please choose a different one !' });
            }
        ...
        // If profile photo is provided, upload it to cloudinary and update photo
        if (profilePhoto) {
...
const updatePassword = async (req, res) => {
    try {
        ...
        // Handle the new password and assign to current user password
